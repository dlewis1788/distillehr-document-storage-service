...
import com.projectivesoftware.distillehr.domain.empi.IdentifierDomain;
import com.projectivesoftware.distillehr.domain.empi.Person;
import com.projectivesoftware.distillehr.domain.empi.PersonIdentifier;
...
public class EmpiService {
    ...
    public EmpiService(@Value("${com.projectivesoftware.distillehr-document-storage-service.empi-service-url}") String empiServiceUrl,
                       @Value("${com.projectivesoftware.distillehr-document-storage-service.empi-session-key}") String empiSessionKey,
                       @Value("${com.projectivesoftware.distillehr-document-storage-service.empi-target-identifier-domain-name}") String empiTargetIdentifierDomainName) {
            ...
            if (personResponse == null) {
                ...
                for (PersonIdentifier personResponseIdentifier : personResponse.getPersonIdentifiers()) {
                    if (Objects.equals(personResponseIdentifier.getIdentifierDomain().getIdentifierDomainName(), empiTargetIdentifierDomainName)) {
                        if (Objects.equals(personResponseIdentifier.getIdentifier(), personIdentifier.getIdentifier())) {
                            return true;
                        } else {
                            return false;
                        }
